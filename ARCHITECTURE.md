...
| **7 (shipped, v1)** | In-browser realtime render: ExportEngine plays the source through a hidden element, composites each frame onto a canvas at the frame-ratio resolution (1080×1920 for 9:16 etc.) via computeDrawRect() — a unit-tested pure mirror of object-fit cover/contain + object-position + punch-in scale — skips cut ranges live, draws animation overlays in canvas (levels, preset colors, enter-motion approximations), routes source audio through an AudioContext into the recording, and captures with MediaRecorder (VP9/Opus WebM, or MP4 where the browser records it, 8 Mbps). Save-file flow with sandbox-blocked-download honesty. E2E-verified: real 150 KB VP9+Opus file rendered from the test clip with the cut skipped. Later upgrade path: frame-accurate server render (registry entry kept, honestly gated). | Browser (MediaRecorder) |
