...
| **2** | Text-intelligence backend online (Claude API via thin server): hook AI scoring + alternatives, angle reasoning, brief synthesis, Content Map generation from brief, chat NLU on top of the router | AI backend |
| **3** | Transcription + footage analysis + EDL generation; multi-clip timeline model | STT (e.g. Whisper), scene detection, audio analysis + AI |
| **4** | Animation Sequencer: visual sequencing lane, hierarchy enforcement, on-preview overlay rendering | Builds on P2/P3 |
